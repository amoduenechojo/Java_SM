...
//        input.(priceOfItemsBought.add(2)); 
        ...
        for (int firstCounter = 0; firstCounter < pieces.size(); firstCounter++){
		...
		for (int counter = 0; counter < itemsBought.size(); counter++){

			System.out.printf("%16s%8d%16f%20f%n", itemsBought.get(counter), pieces.get(counter), price.get(counter), (pieces.get(counter) * price.get(counter)));
       ...
       if (amount >= totalBill){
        ...
        System.out.printf("%13s%5d%14.2f%20.2f%n", itemsBought.get(counter), pieces.get(counter), price.get(counter), (total = (pieces.get(counter) * price.get(counter))));
        }
        }
